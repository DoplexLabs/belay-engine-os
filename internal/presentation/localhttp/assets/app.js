(() => {
  ...
  function renderReportHabits() {
    ...
    const waitingIssue = topReportIssue();
    let patternShown = false;
    const cards = document.createDocumentFragment();
    sessions.forEach((session) => {
      const showPattern = !patternShown && !session.debrief && Boolean(waitingIssue);
    ...
    if (sessions.length === 0 && waitingIssue) {
      elements.reportHabitsEmpty.appendChild(createWaitingPattern(waitingIssue));
    }
    renderReportHabitSummary(ready);
  }

  function topReportIssue() {
    const issues = state.developerBrief && state.developerBrief.top_issues;
    return Array.isArray(issues) && issues.length > 0 ? issues[0] : null;
  }

  function createWaitingPattern(issue) {
    const block = createElement("div", "report-habit-pattern");
    const metrics = [
      formatIssueDollarCost(issue.cost),
      formatIssueMinutes(issue.cost),
      `${formatNumber(issue.session_count)} ${toFiniteNumber(issue.session_count) === 1 ? "session" : "sessions"}`,
    ...
    block.append(
      ...
      createElement(
        "h3",
        "report-habit-pattern-title",
        humanizeReportHeadline(issue.headline),
      ),
      createElement("p", "report-habit-pattern-metrics", metrics),
    );
    if (issue.excerpts.length) {
      block.append(createReportIssuePreview(issue.excerpts[0]));
    }
    const evidenceButton = createElement(
      "button",
      "secondary-button",
  ...
  function renderReportHabitCard(session, harness, showPattern) {
    ...
    if (!record) {
      ...
      if (showPattern) {
        ...
        if (issue) card.appendChild(createWaitingPattern(issue));
      }
      return card;
    }
